...
Tööl on siiski teoreetilised lähtekohad, mis tulenevad ühelt poolt arvutimorfoloogia nõuetest ja teisalt klassikalisest paradigmaatilisest morfoloogiakäsitlusest. Järgmiselt püüan argumenteerida, et arvutimorfoloogia ei pea olema lingvistisest teooriast ajendatud. Seejärel tutvustan tööle kõige lähedamini asetsevat morfoloogilist käsitlust.
...
Sellist vormisõnastikku võib kujutada erinevatel viisidel. Näiteks leksikaalse andme\-baasina, kus iga lekseemi puhul on nenditud kõik selle muutevormid koos vastava grammatilise iseloomustusega. Matemaatilises mõttes kujutab vormisõnastik vaid \textit{seost} muutevormide ja nende vastavate grammatiliste iseloomustuste vahel. 
...
Arvutimorfoloogiad võivad seda seost, või vormisõnastiku funktsionaalsust, realiseerida arvutuslikult erinevatel viisidel ja ei pea olema lingvistiliselt motiveeritud. Aga ühe täieliku vormisõnastiku koostamise maht on niivõrd suur (kui mitte lõpmatult suur) ja see on peamine motivatsioon organiseerida vormisõnastiku arvutusliku külje ühel või teisel moel lingvistiliselt.
...
Eelmise sajandi keskpaiku jagas Charles Francis Hockett kõik seni Ameerikas sajandi algusest saadik ilmunud grammatikad kahe üldise mudeli järgi, IA (\textit{Item-and-Arrangement}, üksus ja distributsioon, üksus ja järjestus) ja IP (\textit{Item-and-Process}, üksus ja protsess ehk protsessi\-morfoloogia). Kõrvalmärkusena tõi ta välja ka kolmanda, ``vanema ja väärikama'' mudeli, WP (\textit{Word-and-Paradigm}, sõna ja paradigma), aga jätab selle oma käsitlusest välja (\cite{hockett_two_1954} lk 210). Hockett võrdleb IA ja IP mudelite eeliseid ja argumenteerib, et IA populaarsus seisneb eeskätt sellel, et ajastu eelistab formaalseid mudeleid. Kuna IA-mudel oli juba formaliseeritud tahtis Hockett nüüd formaliseerida IP-mudeli (\cite{hockett_two_1954}, lk 214) ning sellest sai hiljem Fred Karlssoni sõnade järgi generatiivse lingvistika peamiseks mudeliks (\cite{karlsson_uldkeeleteadus_2002} lk 126).

IP-mudel põhineb (morfoloogilise) protsessi mõistel, millega ühest algvormiks valitud kujust (\textit{base}) luuakse teine vorm (\cite{hockett_two_1954} lk 210). IA tekkis vastureaktsioonina IP protsessimõiste suunalisusele -- enam ei tahetud tõsta esile üht vormi algsemaks teistest vormidest (\cite{hockett_two_1954} lk 211). IA põhineb morfeemi mõistel, mida Hockett kirjeldab kui keele väikseimat grammatiliselt olulist üksust, ja selle distributsiooni määramisel (\cite{hockett_two_1954} lk 212). Hocket nendib, et ka IA mudeli puhul tuleb siiski teha kohati suvalisi valikuid selle üle, mis kuulub morfeemi tasandile ja mis kuulub distributsiooni tasandile (\cite{hockett_two_1954} lk 212).

% Stumpi neljamõõtmeline jaotus
Gregory Stump on arendanud Hocketti IP ja IA kaheksjagamise klassifikatsiooni edasi tänapäevaste morfoloogiliste teooriate põhjal. Nimetades IAd leksikaalseks (ingl. \textit{lexical}) ja IPd inferentsiaalseks (ingl. \textit{inferential}) on ta lisanud klassifikatsioonile veel sisemise telje: inkrementaalsed (ingl. \textit{incremental}) ja realiseerivad (ingl \textit{realizational}) teooriad. (\cite{stump_inflectional_2001}, lk 1-2)

Inkrementaalsete teooriate järgi lisandub iga (olgu IA järgi leksikaalselt loetletud või IP mõttes inferentsiaalse reegliga tuletatud) morfosüntaktilise tunnuse puhul sõnale ka selle vormiline eksponent (\cite{stump_inflectional_2001}, lk 2). Vormilised eksponendid on üks-üheses seoses grammatiliste tunnustega ja need väljenduvad ükshaaval ehk inkrementaalselt.

Realiseerivate teooriate juures ei pea eksponent iga morfosüntaktilise tunnuse puhul eraldi ja koheselt väljenduma, vaid vormiline väljendus võib realiseeruda tunnuste suuremate komplektide puhul või üldse kui sõna kõik tunnused on teada (\cite{stump_inflectional_2001}, lk 2).

Realiseerivad teooriad võimaldavad niisiis suurema paindlikkuse vormiliste väljendujate \textit{realiseerimisel}, loobudes vormiliste väljendujate üks-ühesest seosest morfosüntaktiliste tunnustega.

Stumpi jagab oma klassifikatsiooni järgi Lieberi morfoloogilise teooria leksikaalseks ja inkementaalseks. Halle ja Marantzi distributsioonilise morfoloogia teooria leksikaalseks ja realiseerivaks. Steele'i artikuleeritud morfoloogia teooria esindab inferentsiaalset ja inkrementaalset suunda.

Stumpi enda ja Matthewsi, Zwicky ning Andersoni teooriaid nimetab ta WP teooriateks, mis on inferentsiaalsed ja realiseerivad (\cite{stump_inflectional_2001}, lk 3).
...
Robert Beard on nimetanud ülaltoodud autorite arendatud realiseerivaid teooriaid ird-morfoloogiaks (\textit{ingl 'split' morphology}) (\cite{beard_morpheme_1987}, lk 20) ja pakkunud välja morfoloogia veel võimsama eraldamise, mis põhineb tema morfoloogia lahususe hüpoteesil (\textit{Separation Hypothesis}) (\cite{beard_lexeme-morpheme_1995}).

% ird-morfoloogia VS morfoloogia lahususe hüpotees
Morfoloogia lahususe hüpoteesil põhinevate teooriate ja realiseerivate (ird-)morfoloogia\-teooriate vahe on fundamentaalne ja lähtub nende käsitlusest süntaksi ja semantika vahekorrast. Kõige ilmekalt paistab nende vahe morfeemi definitsioonis, küsimuses kas morfeem on keele väikseim vormiline tähenduslik üksus või mitte.
...
Käesolevas magistritöös rakendatud ekstraktmorfoloogia on oma organisatsiooni suhtes WP mudel, aga selle käsitus morfeemist on lähedasem Beardi teooriale.
...
Kusjuures (1)\nobreakspace viitab otseselt klassikalisele morfeemikäsitusele.

Karttuneni artikkel on aga ajendatud Stumpi teooriast ja ta näitlikustab kuidas Stumpi teooria on võimalik rakendada kasutades lõplike automaatide formalismi.

Karttunen toob siiski välja olukorra, et arvutilingvistide morfoloogiad põhinevad arvutuslikel meetoditel, millega nad implementeerivad morfoloogiaid ja mitte ei põhine otse mingil teoorial. Seega võib öelda, et arvutimorfoloogia on laiem kui lingvistiline morfoloogia, kuna esimest piirab mitte teooria, vaid arvutusliku meetodi võimsus. Karttunen näitabki, et Stumpi inferentsiaalne-realiseeriv teooria on taandatav lõplike automaatide formalismi arvutusvõimsusele. (\cite{karttunen_computing_2003})

% TODO lisada ka tema ütlus Stumpi teooria motiveerituses olla teistele morfologistidele elegantne

Sellest võib järeldada, et arvutilingvistikas on lingvistilise teooria roll pigem olla ajendiks kui tõetruuks postulaadiks, kuigi kindlasti on teooria ja selle implementatsioonilise praktika vahekord raskesti eraldatavad ja ajas muutuvad. Tendentsi tõetruuduse vähenemisele võib siiski täheldada ka Kimmo Koskenniemi töös, kus ta on hiljuti oma taandatud kahetasemelises morfoloogiamudelis püüdnud morfofoneemi mõiste juures loobuda selle tähendusliku külje lingvistilisest realismist, omastades seda puhtalt vormile:
\begin{quote}
